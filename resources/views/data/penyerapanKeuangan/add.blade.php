...
<div class="container-admin">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" id="mainForm" name="mainForm" action="{{ route('simpanDataPenyerapanKeuangan') }}" class="hide">
                        @csrf
                        <div class="row">
                        ...
                        @if(!empty($pakets))
                            <div class="col-12 col-md-12">  
                                <div class="form-group">
                                    <label for="mak">Paket Sub Kegiatan - MAK</label>
                                    <select class="form-control" name="id_mak" id="mak" required>
                                        <option value="" style="background-color: #111;color: #fff;"></option>
                                    @foreach($maks as $res)
                                        <option value="{{ $res->id }}">{{ $res->paket->paket }}. {{ $res->paket->keterangan_paket }} - {{ $res->mak }}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        @endif
                            ...
                            <div class="col-12 col-md-12">  
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="realisasi_kegiatan" required name="realisasi_kegiatan">
                                </div>
                            </div>
                            <div class="col-12 col-md-12">  
                                <div class="form-group">
                                    <label for="realisasi_nilai">Realisasi Nilai (IDR)</label>
                                    <input onkeypress="return isNumberKey(event)" type="text" class="form-control" id="realisasi_nilai" required name="realisasi_nilai">
                                </div>
                            </div>
                        </div>
